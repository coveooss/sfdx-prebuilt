...
function getVersionDefinitionFromString(versionString) {
  var matches = /(\d+\.\d+\.\d+)-/.exec(versionString)
  var versionParts = matches[1].split('.')

  return { major: versionParts[0], minor: versionParts[1], revision: versionParts[2] }
}

function isVersionEqualOrGreater(testVersion, targetVersion) {
  var test = getVersionDefinitionFromString(testVersion)
  var target = getVersionDefinitionFromString(targetVersion)

    
  if(target.major > test.major) {
    return false
  } else if(target.major < test.major) {
    return true
  } else if(target.minor > test.minor) {
    return false
  } else if(target.minor < test.minor) {
    return true
  } else if(target.revision > test.revision) {
    return false
  } else {
    return true
  }
...
function checkSFDXVersion(sfdxPath, version) {
  ...
  return kew.nfcall(cp.execFile, sfdxPath, ['--version']).then(function (stdout) {
    ...
    if (isVersionEqualOrGreater(actualVersion, version)) {
      return true
    } else {
